...
void button_getButton() {
	...
	// look like:
//	GPIO_PORTE_DATA_R => 0b0000 0000; //if Sw3 is pushed
//	GPIO_PORTE_DATA_R => 0b0000 0100; //if Sw3 is not pushed
	//
	// We want to only look at bit 2 and mask the other 7 bits:
	//
	// Bitwise AND:
	 if(((GPIO_PORTE_DATA_R >> 0) & 1) == 0) {
	     printString("\rButton 1 pushed\n");
	     //sw = '1'; //if Sw3 is pushed
	 }
	 else {
	    // printString("\rButton 1 not pushed\n");
	     //sw = '0'; //if Sw3 is not pushed
	 }

	 if(((GPIO_PORTE_DATA_R >> 1) & 1) == 0) {
      printString("\rButton 2 pushed\n");
      //sw = '1'; //if Sw3 is pushed
    }
    else {
      //printString("\rButton 2 not pushed\n");
      //sw = '0'; //if Sw3 is not pushed
    }

	 if(((GPIO_PORTE_DATA_R >> 2) & 1) == 0) {
      printString("\rButton 3 pushed\n");
      //sw = '1'; //if Sw3 is pushed
    }
    else {
      //printString("\rButton 3 not pushed\n");
      //sw = '0'; //if Sw3 is not pushed
    }

    if(((GPIO_PORTE_DATA_R >> 3) & 1) == 0) {
      printString("\rButton 4 pushed\n");
      //sw = '1'; //if Sw3 is pushed
    }
    else {
      //printString("\rButton 4 not pushed\n\n\n\n\n");
      //sw = '0'; //if Sw3 is not pushed
    }
